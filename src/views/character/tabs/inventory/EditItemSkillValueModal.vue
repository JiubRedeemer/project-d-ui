<script setup lang="ts">
import {IonButton, IonIcon, IonModal} from "@ionic/vue";
import {useRoute} from "vue-router";
import {ref} from "vue";
import {checkmarkOutline} from "ionicons/icons";
import {useCharacterStore} from "@/stores/CharacterStore";
import {ItemSkill} from "@/components/models/response/InventoryResponse";

const route = useRoute();

const props = defineProps({
  itemSkill: ref<ItemSkill>,
  characterId: String,
  isOpen: Boolean, // Принимаем видимость модалки
});

const characterStore = useCharacterStore()



const emit = defineEmits(["closeEditItemSkillModal"]); // Добавляем событие закрытия


async function onSubmit() {

}

</script>

<template>
  <ion-modal
      :is-open="isOpen"
      @didDismiss="emit('closeEditItemSkillModal')"
      :initial-breakpoint="1"
      :breakpoints="[0, 0.5, 1]"
  >
    <div class="block">
      <div class="header">
        <div class="name">TEST
        </div>
      </div>
      <div class="input-block">
      </div>
      <div class="footer">
        <ion-button size="large" shape="round" @click="onSubmit">
          <ion-icon slot="icon-only" :icon="checkmarkOutline" color="onPrimary"></ion-icon>
        </ion-button>
      </div>
    </div>
  </ion-modal>
</template>


<style scoped>
.block {
  width: 100%;
  height: 300px;
  display: flex;
  justify-content: space-between;
  flex-direction: column;
}

.header {
  padding: 10px;
  font-size: 1.5rem;
  width: 100%;
  display: flex;
  justify-content: start;
  flex-direction: row;
}

.input-block {
  padding: 10px;
  display: flex;
  justify-content: center;
}

.footer {
  padding: 10px;
  display: flex;
  justify-content: end;
}

ion-modal {
  --border-radius: 10px;
  --height: auto;
  --width: 90%;
  --background: var(--ion-color-dark);
}
</style>
