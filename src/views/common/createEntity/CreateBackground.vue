...
import {getAbilitiesForRoom} from "@/api/rulebookApi";
...
function slugCode(name: string): string {
  ...
  const existing = new Set(roomCreationStore.backgrounds.map((b) => b.code));
...
async function uploadBackgroundImage(file: File): Promise<string> {
  ...
  formData.append("userFilename", createBackgroundStore.background.id || createBackgroundStore.background.code || "background");
...
onBeforeMount(async () => {
  ...
  const roomId = createBackgroundStore.roomId ?? roomCreationStore.roomInfoCreatedId;
  if (roomId) {
    try {
  ...
  <ion-page>
    ...
    <ion-content color="dark">
      ...
      <ion-buttons class="buttons-block">
        ...
        <ion-button color="primary" fill="solid" shape="round" :disabled="!createBackgroundStore.background.name?.trim()" @click="onSave">
