...
function slugCode(name: string): string {
  ...
  const existing = new Set(useGuidebookStore().classes.map((c) => c.code));
...
onBeforeMount(async () => {
  ...
  const roomId = createClassStore.roomId ?? useGuidebookStore().roomId;
  const baseRules = useGuidebookStore().baseRuleType;
  if (roomId) {
    try {
